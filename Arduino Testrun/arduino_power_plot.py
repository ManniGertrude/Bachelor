...
def arduino_power_plot(path):
    pdf_path = f'{path}/arduino_power_plot.pdf'
    with PdfPages(pdf_path) as pdf:
        for root, dirs, files in os.walk(path):
            ...
            if len(csv_files) > 0:
                ...
                ax1.grid()
                plt.legend(['Temp = 30 °C', 'HV = 5V'], loc='lower right', markerscale = 0, framealpha=0.5)
                pdf.savefig(fig, bbox_inches='tight')
                plt.close(fig)
    print(f"Plots saved to {pdf_path}")

arduino_power_plot(pathtest)