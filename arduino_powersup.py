...
def DAC_comparison_plot(path, infostring=None, save = True):
    TEXT_COLOR = '#07529a'
    TITLE_COLOR = '#07529a'
        ...
        for root, dirs, files in os.walk(path):
            if root[-9:] == "dac_power":
                ...
                for lower_root, lower_dirs, lower_files in os.walk(root):
                    ...
                    if len(csv_files) != 0 and counter < 10:
                        counter += 1
                        df = pd.read_csv(f'{lower_root}/{csv_files[-1]}', skipinitialspace=True)
                        print(f"Processing file: {lower_root}/{csv_files[-1]}")
                        fig, ax1 = plt.subplots()
                        ax2 = ax1.twinx()
...
try:
    ...
    start = time.time()
    DAC_comparison_plot(test_path, infostring = 'DAC comparison')
    end = time.time()
